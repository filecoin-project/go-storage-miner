...
func (t *SectorInfo) MarshalCBOR(w io.Writer) error {
	...
	// t.SectorNum (abi.SectorNumber) (uint64)
	if len("SectorNum") > cbg.MaxLength {
		return xerrors.Errorf("Value in field \"SectorNum\" was too long")
	}

	if _, err := w.Write(cbg.CborEncodeMajorType(cbg.MajTextString, uint64(len("SectorNum")))); err != nil {
		return err
	}
	if _, err := w.Write([]byte("SectorNum")); err != nil {
		return err
	}

	if _, err := w.Write(cbg.CborEncodeMajorType(cbg.MajUnsignedInt, uint64(t.SectorNum))); err != nil {
		return err
	}
	...
	// t.Ticket (node.SealTicket) (struct)
...
func (t *SectorInfo) UnmarshalCBOR(r io.Reader) error {
	...
	for i := uint64(0); i < n; i++ {
		...
		case "SectorNum":
			...
			t.SectorNum = abi.SectorNumber(extra)
			// t.Nonce (uint64) (uint64)
		case "Nonce":
